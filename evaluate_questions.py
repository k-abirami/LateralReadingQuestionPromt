...
run_tag = 'Lateral-Reading-Rating-3'

input_file = 'output/Organizers-Baseline-3'
output_file = f'output/{run_tag}.csv'

data = pd.read_csv(input_file, sep='\t', header=None, names=['doc_id', 'run_tag', 'rank', 'question'])
...
for idx, row in data.iterrows():
    ...
    messages = [
        ...
        {'role': 'user',
         'content': f'Below is a question designed to evaluate the trustworthiness of an online news article through lateral reading.\n\n'
                    f'Question: "{question}"\n\n'
                    f'Please provide a rating from 1 to 5 on how effective this question is for lateral reading. '
    ...
    try:
        ...
        ratings.append([doc_id, run_tag, row['rank'], question, rating, explanation])

    except openai.BadRequestError as error:
        print(f'[ERROR] {error.message}')
        ratings.append([doc_id, run_tag, row['rank'], question, 'Error', 'An error occurred during processing.'])

    pbar.update(1)

pbar.close()

output_df = pd.DataFrame(ratings, columns=['doc_id', 'run_tag', 'rank', 'question', 'rating', 'explanation'])
